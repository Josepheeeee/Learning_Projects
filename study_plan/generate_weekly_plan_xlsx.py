...
first_8_weeks_detail = [
    [1, "当前进度后连续5节", "数组、复杂度、二分基础、递归入门、简单排序思想", "两数之和；合并两个有序链表；有效的括号；二叉树的中序遍历；买卖股票的最佳时机；最大子数组和", "数组前半：二分查找、移除元素、有序数组的平方、长度最小的子数组", "项目A 路径搜索可视化：建目录、画网格、定义起终点和障碍、跑通 BFS", "Kaggle 仅做浏览：收藏 3-5 个 tracking / trajectory / frame prediction notebook", "最小可运行网格搜索脚本 + BFS 笔记"],
    [2, "再连续5节", "链表、栈、队列、哈希表、双指针", "反转链表；环形链表；最长连续序列；三数之和；盛最多水的容器；无重复字符的最长子串", "链表前半：移除链表元素、反转链表、两两交换节点、删除倒数第N个结点", "项目A 路径搜索可视化：加入 DFS、随机障碍、地图编辑、对比 BFS/DFS", "选 1 个体育 tracking 或轨迹预测 notebook，记录输入/标签/baseline", "BFS/DFS 对比可视化 + README 初稿"],
    [3, "再连续5节", "树、堆、优先队列、递归深入、二叉树遍历", "二叉树的层序遍历；验证二叉搜索树；对称二叉树；翻转二叉树；二叉树的最大深度；从前序与中序构造二叉树", "二叉树前半：前中后序遍历、层序遍历、翻转二叉树、对称二叉树", "项目A 收尾：加入 Dijkstra；项目B 启动：任务调度/拓扑排序系统，定义任务节点格式", "选定第一个 Kaggle 方向，并写 1 页项目分析", "路径搜索项目 v1 + 算法对比图"],
    [4, "再连续5节", "图基础、BFS、DFS、拓扑排序、并查集/连通性", "岛屿数量；课程表；腐烂的橘子；克隆图；实现 Trie；单词搜索", "图论前半：岛屿问题、所有可达路径、并查集入门、拓扑排序相关题", "项目B 任务调度系统：输入依赖图、输出执行顺序、加入优先级、可视化依赖", "正式开始 Kaggle：拆 1 个 notebook 的结构，记录特征/baseline/指标", "调度系统最小版 + 拓扑排序笔记"],
    [5, "再连续5节", "最短路、贪心、图优化、堆在图中的应用、DP 引入", "零钱兑换；爬楼梯；打家劫舍；最长递增子序列；组合总和；跳跃游戏", "贪心前半：分发饼干、摆动序列、最大子数组和、跳跃游戏", "项目B 收尾；项目C 启动：二维物理规则模拟器，先定义状态和边界", "开始第一个 Kaggle baseline notebook：数据读取 + 可视化 + 最简单 baseline", "调度系统 v1 + 物理模拟器状态设计文档"],
    [6, "再连续5节", "DP 基础、状态定义、转移方程、记忆化/递推、经典DP 入门", "不同路径；最小路径和；编辑距离；最长公共子序列；分割等和子集；单词拆分", "DP 前半：斐波那契、爬楼梯、最小花费爬楼梯、不同路径", "项目C 二维物理规则模拟器：小球移动、边界碰撞、时间步更新、多次 rollout 保存", "完成 Kaggle baseline 第一版，并产出 2-3 张图", "可连续播放的小球运动模拟器 + rollout 笔记"],
    [7, "再连续5节", "DP 进阶、背包、区间/序列DP、递归到DP的转换、综合题型", "最长回文子串；最长有效括号；乘积最大子数组；完全平方数；目标和；单词拆分（复刷）", "DP 后半：01背包基础、分割等和子集、最后一块石头的重量II、目标和", "项目C 二维物理规则模拟器：加入重力、多个物体、简化碰撞、导出状态序列", "改进 Kaggle baseline，并把 notebook 写成：背景/数据/baseline/结果", "可导出数据的物理模拟器 v1"],
    [8, "再连续5节", "综合复盘、高频题型、复杂度复习、模板归纳、阶段总结", "LRU 缓存；搜索二维矩阵II；接雨水；旋转图像；合并区间；每日温度", "回顾前7周错题；图/树/DP 各补1-2题", "项目C 收尾；项目D 启动：StateWorld-Predictor，确定输入输出并做一步预测 baseline", "完成第一个 Kaggle 项目第一版 notebook，形成公开展示结构", "阶段1项目总复盘 + world model 项目准备"],
]

kaggle_projects = [
    ["K1", "体育追踪/行为预测", "高", "2-3 周", "tracking data、时空行为、可视化反馈强", "适合作为第一个 Kaggle 主项目"],
    ["K2", "轨迹预测/自动驾驶类", "高", "3-4 周", "trajectory forecasting、多主体状态预测", "和 world model / 驾驶场景最接近"],
    ["K3", "视频/帧序列预测", "中-高", "3-5 周", "future frame prediction、sequence modeling", "适合作为视频型 world model 过渡项目"],
    ["K4", "纯 tabular 排行赛", "低", "1-2 周", "反馈快，但和 world model 相关性弱", "只建议作为补充"],
]

agent_skills = [
    ["Prompt 拆解", "每周2次，每次20-30分钟", "把任务拆成目标、输入输出、模块、边界条件", "提高你用 AI 辅助开发的效率"],
    ["代码审查", "每周2次，每次20-30分钟", "检查 AI 生成代码的复杂度、边界和正确性", "防止被 AI 胡说带偏"],
    ["AI 工作流", "每周1-2次", "让 AI 帮你搭目录、写测试、补 README、重构", "把 AI 当工程助手，而不是替代学习"],
]

prompt_engineering_resources = [
    ["Prompt结构化", "Anthropic Prompt Engineering / 官方文档与示例", "任务拆解、上下文组织、约束表达", "先学会把需求写清楚，再让 AI 写代码"],
    ["Prompt结构化", "OpenAI Prompting Guide / 官方最佳实践", "角色、目标、输入输出格式、few-shot", "适合练 notebook 指令和数据分析任务描述"],
    ["工程化表达", "System Design Primer（GitHub）", "模块拆分、接口意识、边界条件", "不要求现在学大系统，先学清晰拆模块"],
    ["工程化表达", "Refactoring Guru", "重构意识、代码味道、命名与结构", "适合配合 AI 生成代码后做二次整理"],
    ["Notebook写作", "Kaggle Learn: Intro to Machine Learning / Pandas / Data Visualization", "标准 notebook 结构、图表、结论表达", "最适合你当前场景的 notebook 入门材料"],
    ["Notebook写作", "Kaggle 高票 Notebook（tracking / trajectory 类）", "背景、EDA、baseline、结果、结论", "重点模仿结构，不是照抄模型"],
    ["实验记录", "Weights & Biases Reports / Blog", "实验记录、结果对比、图表讲述", "学习怎么把实验过程写得像项目"],
    ["代码质量", "pytest 官方文档", "最小测试、回归测试、边界样例", "帮助你把 AI 生成脚本变成可维护项目"],
]

daily_schedule = [
    ["08:00-09:30", "B站算法主课", "当天最需要脑子的主线内容"],
    ["09:30-10:20", "Hot100 1题", "建立面试题感"],
    ["10:20-11:00", "错题复盘 / 代码随想录", "巩固当天算法主题"],
    ["13:30-14:30", "代码随想录专题刷题", "跟算法主课专题同步"],
    ["14:30-15:30", "Kaggle 项目分析 / notebook 阅读", "做反馈型项目主线"],
    ["15:30-16:30", "Prompt 结构化训练 / 工程化表达 / AI辅助编程训练", "优先练任务拆解、模块设计、Kaggle notebook 结构；论文与GitHub阅读放次选"],
    ["20:30-21:15", "低脑力任务", "流程图、TODO、复盘；README 放到项目已有雏形后再做"],
    ["21:15-22:30", "个人电脑项目编码", "优先主线项目，再做 Kaggle notebook"],
]
...
detail8.append(["周次", "B站课进度", "本周核心主题", "Hot100", "代码随想录", "项目推进", "Kaggle并行", "本周输出"])
for row in first_8_weeks_detail:
    detail8.append(row)
style_sheet(detail8)
for col, width in {"A": 8, "B": 16, "C": 28, "D": 52, "E": 42, "F": 42, "G": 38, "H": 30}.items():
    detail8.column_dimensions[col].width = width
detail8.freeze_panes = "A2"

# Sheet 9: Kaggle项目建议
kag = wb.create_sheet("Kaggle项目建议")
kag.append(["编号", "项目类型", "适合度", "宽松时间", "你会练到什么", "当前建议"])
for row in kaggle_projects:
    kag.append(row)
style_sheet(kag)
for col, width in {"A": 8, "B": 24, "C": 10, "D": 12, "E": 40, "F": 30}.items():
    kag.column_dimensions[col].width = width
kag.freeze_panes = "A2"

# Sheet 10: AI辅助编程
agent = wb.create_sheet("AI辅助编程")
agent.append(["能力", "建议频率", "练什么", "目的"])
for row in agent_skills:
    agent.append(row)
style_sheet(agent)
for col, width in {"A": 16, "B": 18, "C": 40, "D": 34}.items():
    agent.column_dimensions[col].width = width
agent.freeze_panes = "A2"

# Sheet 11: Prompt工程化资源
prompt_ws = wb.create_sheet("Prompt工程化资源")
prompt_ws.append(["类别", "资源", "建议重点", "为什么现在学"])
for row in prompt_engineering_resources:
    prompt_ws.append(row)
style_sheet(prompt_ws)
for col, width in {"A": 16, "B": 44, "C": 34, "D": 36}.items():
    prompt_ws.column_dimensions[col].width = width
prompt_ws.freeze_panes = "A2"

# Sheet 12: 每日时间安排
sch = wb.create_sheet("每日时间安排")
sch.append(["时间段", "建议任务", "说明"])
for row in daily_schedule:
    sch.append(row)
style_sheet(sch)
for col, width in {"A": 16, "B": 28, "C": 42}.items():
